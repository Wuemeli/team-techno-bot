...
if (context.params.event.member.roles.includes(base.rows[0].fields.Member))
  return lib.discord.interactions['@release'].followups.ephemeral.create({token, content: `**${event.member.user.username}**, are already verified.`});
if (base.rows[0].fields.Type == `button`) {
  await lib.discord.guilds['@release'].members.roles.update({role_id: base.rows[0].fields.Member, user_id: event.member.user.id, guild_id});
  await lib.discord.interactions['@release'].followups.ephemeral.create({token, content: `**${event.member.user.username}**, you have been verified.`});
} else if (base.rows[0].fields.Type == `captcha`) {
  let capCheck = await lib.airtable.query['@1.0.0'].select({table: `Captcha`, where: [{'User__is': event.member.user.id, 'Guild__is': guild_id}]});
  if (capCheck.rows.length) 
    return lib.discord.interactions['@release'].followups.ephemeral.create({token, content: `**${event.member.user.username}**, you already have a captcha.`});
